...
### ⚠️ 机器环境故障：SwapChainPanel 屏幕内容不可见（2026-09-19 发现，待查）

* **现象**：本机（Lenovo R7000，Win11 build 26200）自 2026-09-19 起，SwapChainPanel 的
  合成交换链内容**不再出现在屏幕上**（面板区域透底，XAML 装饰如边框/标题条正常）。
  M1.4 warp 渲染路径与 M0.2 blit 路径**同样受影响**；**改动前基线 baaf444（9/16 时面板可见）
  当日构建复跑同样透底** → 非 M1.4 回归，是 9/16 之后机器环境变化。
* **渲染侧已独立举证为健康**：48 FPS 出帧、后备缓冲 staging 回读 alpha=255 不透明镜像 +
  常量正确 + `--warp-selftest` 9 档边界 vs 单应预测 ≤3px 无滞回（DD-039 §7）——
  故障发生在「合成交换链 → DWM → 屏幕」一段，App 内部管线正常。
* **时间线证据**：KB5129195（安全更新）InstalledOn=2026-09-15，机器 LastBootUpTime=
  2026-09-19 07:20（本次会话前最后一次重启）；9/16 验收时面板可见 → 待更新在 9/19 重启
  生效的假设与全部观察吻合。同期显示拓扑变化：主屏 1920×1080 + 副屏（1920,0），
  系统 DPI 缩放 125%（9/16 验收坐标反推为 100%——缩放是否被动过待用户确认）。
* **对里程碑的影响**：屏幕级视觉验收（截图/CopyFromScreen 类）在本机暂时不可信；
  渲染类里程碑改用进程内回读验收（`--warp-selftest` 套路，DD-039），已验证可行且更精确。
  **M1.8 全链路屏幕级视觉验收前必须解决本条**（候选动作：卸载 KB5129195 对照 / 干净
  账户或安全模式对照 / 回滚 WinAppSDK 1.8.* 到 9/16 版本 / 用户确认 DPI 是否改过）。
